...
static constexpr int8_t MAX_SCALE_INPUT = 90;
...
Vibrator::Vibrator(std::ofstream&& activate, std::ofstream&& duration, std::ofstream&& effect,
        std::ofstream&& queue, std::ofstream&& scale, std::uint32_t q_index,
        std::vector<uint32_t>&& v_levels) :
...
Return<Status> Vibrator::on(uint32_t timeoutMs, uint32_t effectIndex) {
    ...
    if (effectIndex == WAVEFORM_LONG_VIBRATION_EFFECT_INDEX) {
        setAmplitude(toAmplitude(mVolLevels[WAVEFORM_LONG_VIBRATION_EFFECT_LEVEL]));
    }
...
Return<void> Vibrator::performEffect(Effect effect, EffectStrength strength,
    ...
    if (effect >= Effect::RINGTONE_1 && effect <= Effect::RINGTONE_15) {
        ...
        switch (strength) {
        case EffectStrength::LIGHT:
            setAmplitude(toAmplitude(mVolLevels[--volIndex]));
            break;
        case EffectStrength::MEDIUM:
            setAmplitude(toAmplitude(mVolLevels[volIndex]));
            break;
...
Return<uint8_t> Vibrator::toAmplitude(uint32_t voltageLevel) {
    return std::round(UINT8_MAX * (voltageLevel/100.0));
}
