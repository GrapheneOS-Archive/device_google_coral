 ...
 */
#define LOG_TAG "android.hardware.vibrator@1.2-service.coral"
...
static constexpr char BEST_Q_CONFIG[] = "q_index";
...
static constexpr char BEST_Q_FILEPATH[] = "/sys/class/leds/vibrator/device/q_index";
...
static bool loadCalibrationData(std::uint32_t& q_index, std::vector<uint32_t>& v_levels) {
    ...
    std::ofstream bestQ{BEST_Q_FILEPATH};
    if (!bestQ) {
        ALOGE("Failed to open %s (%d): %s", BEST_Q_FILEPATH, errno,
                strerror(errno));
    }
    ...
    if(config_data.find(BEST_Q_CONFIG) != config_data.end()) {
        bestQ << config_data[BEST_Q_CONFIG] << std::endl;
        q_index = std::stoul(config_data[BEST_Q_CONFIG]);
    }
...
status_t registerVibratorService() {
    // Calibration data
    std::uint32_t q_index(9);
    std::vector<uint32_t> v_levels= {60, 70, 80, 90, 100, 76};
    ...
    if (!loadCalibrationData(q_index, v_levels)) {
        ALOGW("Failed to load calibration data");
    }

    sp<IVibrator> vibrator = new Vibrator(std::move(activate), std::move(duration),
        std::move(effect), std::move(queue), std::move(scale), q_index, std::move(v_levels));
